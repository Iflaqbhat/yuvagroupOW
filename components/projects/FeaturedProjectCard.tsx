...
export function FeaturedProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  const glowBg = useMotionTemplate`radial-gradient(circle 260px at ${glowX} ${glowY}, hsl(var(--accent) / 0.14), transparent 70%)`;
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col p-6">
        <div className="absolute left-0 top-6 h-[calc(100%-2.5rem)] w-0.5 origin-top scale-y-0 bg-accent transition-transform duration-500 ease-out group-hover:scale-y-100" />
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <h3 className="mt-1 font-display text-2xl tracking-tight transition-colors duration-300 group-hover:text-accent">
              {project.name}
            </h3>
          </div>
          <span className="flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-all duration-300 group-hover:bg-accent group-hover:text-accent-foreground">
