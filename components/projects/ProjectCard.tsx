...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <motion.div
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-sm border border-foreground/10 bg-card transition-all duration-500 hover:border-accent/30 hover:shadow-[0_24px_70px_-44px_hsl(var(--foreground)/0.38)]"
    >
      ...
      <div className="relative flex flex-1 flex-col p-5 md:p-6">
        ...
        <p className="mt-3 flex items-start gap-2 text-[0.95rem] font-medium leading-snug text-muted-foreground">
          <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
          {project.location}
        </p>
        <p className="mt-5 min-h-[6rem] border-t border-foreground/10 pt-5 font-display text-[1.08rem] leading-8 text-foreground/72 line-clamp-3">
          {project.description}
        </p>
      </div>
    </motion.div>
